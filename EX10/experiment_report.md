...
- Using CLI commands makes the workflow repeatable and easy to automate.
...
- Use `pip install -r requirements.txt` so the same packages are installed every time.
...
- Load iris data, train the model, and save it as `model.joblib`.
...
Predict accuracy:
```bash
python cli.py predict --model model.joblib
```

Predict a sample:
```bash
python cli.py predict --model model.joblib --sample "5.1,3.5,1.4,0.2"
```

## 10. Output
- The `train` command prints `Model saved!`.
...
- Explain train command:
  - It trains a random forest model on the Iris data and saves the model file.
- Explain predict command:
  - It loads the saved model and either shows accuracy or predicts one sample.
...
- Explain error handling:
  ...
  - If the model file is missing, the program will show a file error.

## 12. Advantages
- Easy to automate with scripts
- The same steps can be repeated
- Useful for pipelines and simple ML tasks

## 13. Exam Tips
- Remember to install from `requirements.txt`.
- Know the difference between `train` and `predict`.
- Know how to pass sample values with `--sample`.
